...
async function customPage(workersHost) {
    const text = `
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
...
{
  "registry-mirrors": ["https://${workersHost}"]
}
EOF
...
docker pull library/alpine:latest</code><button class="copy-button" onclick="copyCode(this)">复制代码</button></pre>
            <h3>加速拉取镜像命令</h3>
            <pre><code>
docker pull ${workersHost}/library/alpine:latest</code><button class="copy-button" onclick="copyCode(this)">复制代码</button></pre>
        </div>
        <script>
